    ...
    for i in range(num_fields):
        ...
        if field_type == "string":
            ...
            for d in range(depth):
                ...
                current[key] = {}
                current = current[key]
            current["value"] = random.choice(
                [random_string(10), random.randint(1, 1000), True]
            )
...
class MongoHandler:
    ...
    async def setup(self):
        for attempt in range(20):
            try:
                ...
                print(
                    f"[Mongo] Waiting for DB... (Attempt {attempt + 1}/20) Error: {e}"
                )
        ...
        await self.db.unstructured_events.create_index(
            "meta.event_type"
        )  # sparse index на вложенное поле
            ...
            users_batch.append(
                {
                    ...
                    "profile": {
                        ...
                        "settings": {"nested": {"val": random.randint(1, 100)}},
                    },
                    "bookmarks": [
                        {
                            "movie_id": random.randint(0, MOVIES_COUNT),
                            "meta": {"note": random_string(20)},
                        }
                        for _ in range(3)
                    ],
                }
    ...
    async def benchmark(self):
        ...
        start = time.time()
        await self.db.user_likes.insert_one(
            {"user_id": user_id, "movie_id": movie_id, "type": "like"}
        )
        ...
        results.append(
            {"scenario": "Read Unstructured Doc by ID", "time_ms": round(t6, 2)}
        )
        print(f"Read Unstructured Doc by ID: {t6:.2f} ms")

        # 7. Query by nested field (dot notation)
        start = time.time()
        await self.db.unstructured_events.find({"meta.event_type": "click"}).to_list(
            length=50
        )
        ...
        start = time.time()
        await self.db.unstructured_events.find(
            {"payload.data.value": {"$gt": 50}}
        ).to_list(length=50)
...
class PostgresHandler:
    ...
    async def setup(self):
        for attempt in range(20):
            try:
                self.pool = await asyncpg.create_pool(
                    "postgres://user:password@postgres:5432/research"
                )
                print("[Postgres] Connected successfully.")
                break
            except Exception as e:
                print(
                    f"[Postgres] Waiting for DB... (Attempt {attempt + 1}/20) Error: {e}"
                )
                await asyncio.sleep(2)
        else:
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                "DROP TABLE IF EXISTS users, movies, user_likes, action_logs, unstructured_events;"
            )
            await conn.execute("""
    ...
    async def generate(self):
        ...
        async with self.pool.acquire() as conn:
            users_data = [
                (i, '{"theme": "dark"}', '[{"movie_id": 1}]')
                for i in range(USERS_COUNT)
            ]
            await conn.executemany(
                "INSERT INTO users VALUES ($1, $2::jsonb, $3::jsonb)", users_data
            )

            movies_data = [(i, f"Movie {i}", 4.5, 100) for i in range(MOVIES_COUNT)]
            await conn.executemany(
                "INSERT INTO movies VALUES ($1, $2, $3, $4)", movies_data
            )

            remaining = TOTAL_RECORDS - USERS_COUNT - MOVIES_COUNT - UNSTRUCTURED_COUNT
            likes_data = [
                (
                    random.randint(0, USERS_COUNT),
                    random.randint(0, MOVIES_COUNT),
                    "like",
                )
                for _ in range(remaining // 2)
            ]
            await conn.executemany(
                "INSERT INTO user_likes (user_id, movie_id, type) VALUES ($1, $2, $3)",
                likes_data,
            )

            logs_data = [
                (random.randint(0, USERS_COUNT), "click", '{"page": "home"}')
                for _ in range(remaining // 2)
            ]
            await conn.executemany(
                "INSERT INTO action_logs (user_id, action, payload) VALUES ($1, $2, $3::jsonb)",
                logs_data,
            )
            ...
            for i in range(UNSTRUCTURED_COUNT):
                ...
                data_json = {
                    ...
                    if k not in ("_id", "user_id", "timestamp")
                }
    ...
    async def benchmark(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            start = time.time()
            await conn.fetch(
                "SELECT * FROM user_likes WHERE user_id = $1 LIMIT 100", user_id
            )
            t1 = (time.time() - start) * 1000
            results.append({"scenario": "Read User Likes", "time_ms": round(t1, 2)})
            print(f"Read User Likes: {t1:.2f} ms")
            ...
            start = time.time()
            await conn.fetchval(
                "SELECT total_likes FROM movies WHERE id = $1", movie_id
            )
            t2 = (time.time() - start) * 1000
            results.append(
                {"scenario": "Read Movie Likes Count", "time_ms": round(t2, 2)}
            )
            print(f"Read Movie Likes Count: {t2:.2f} ms")
            ...
            start = time.time()
            await conn.execute(
                "INSERT INTO user_likes (user_id, movie_id, type) VALUES ($1, $2, 'like')",
                user_id,
                movie_id,
            )
            await conn.execute(
                "UPDATE movies SET total_likes = total_likes + 1 WHERE id = $1",
                movie_id,
            )
            await conn.fetchval(
                "SELECT total_likes FROM movies WHERE id = $1", movie_id
            ...
            t4 = (time.time() - start) * 1000
            results.append(
                {"scenario": "Write Like + Read Count", "time_ms": round(t4, 2)}
            )
            print(f"Write Like + Read Count: {t4:.2f} ms")
            ...
            t5 = (time.time() - start) * 1000
            results.append(
                {"scenario": "Insert Unstructured Doc", "time_ms": round(t5, 2)}
            )
            print(f"Insert Unstructured Doc: {t5:.2f} ms")
            ...
            t7 = (time.time() - start) * 1000
            results.append(
                {"scenario": "Query by Nested Field", "time_ms": round(t7, 2)}
            )
            print(f"Query by Nested Field: {t7:.2f} ms")
            ...
            t8 = (time.time() - start) * 1000
            results.append(
                {"scenario": "Query Deep Nested Field", "time_ms": round(t8, 2)}
            )
            print(f"Query Deep Nested Field: {t8:.2f} ms")
